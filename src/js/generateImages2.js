...
async function generateImage(title, authors, outputFile) {
    ...
    try {
        ...
        const svg = `<svg width="800" height="600">
            <style>
                .title { fill: white; font-size: 70px; font-family: 'Georgia'; font-weight: bold; }
                .authors { fill: white; font-size: 40px; font-family: 'Arial'; font-weight: bold; }
            </style>
            ${titleLines.map((line, index) => `<text x="100" y="${120 + index * 65}" class="title">${line}</text>`).join('')}
            ${authorLines.map((line, index) => `<text x="150" y="${70 + titleLines.length * 65 + 50 + index * 30}" class="authors">${line}</text>`).join('')}
        </svg>`;
